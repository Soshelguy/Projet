...
const authenticateToken = (req, res, next) => {
  ...
  router.post('/users/register', upload.single('profileImage'), async (req, res) => {
    ...
    try {
        ...
        if (req.file) {
            profileImageUrl = req.file.path; // Cloudinary URL is automatically stored in path
        }
        // Validate input
        ...
        // Prepare response, excluding sensitive information
        const userData = {
            id: userId,
...
router.put('/users/profile', authenticateToken, upload.single('profileImage'), async (req, res) => {
    const client = await pool.connect();
    console.log('Fetching profile for user ID:', req.user.userId); // Log the user ID
...
// Route to handle role assignments
